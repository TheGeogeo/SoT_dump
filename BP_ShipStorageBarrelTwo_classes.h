// BlueprintGeneratedClass BP_ShipStorageBarrelTwo.BP_ShipStorageBarrelTwo_C
// Size: 0x3d8 (Inherited: 0x3c8)
struct ABP_ShipStorageBarrelTwo_C : AActor {
	struct UHitRegSnapshotRedirectImpactToReplicatedMovementAttachParentComponent* HitRegSnapshotRedirectImpactToReplicatedMovementAttachParent; // 0x3c8(0x08)
	struct UStaticMeshComponent* StaticMesh; // 0x3d0(0x08)

	void UserConstructionScript(); // Function BP_ShipStorageBarrelTwo.BP_ShipStorageBarrelTwo_C.UserConstructionScript // Event|Public|BlueprintCallable|BlueprintEvent // @ game+0x18e5fa0
};

